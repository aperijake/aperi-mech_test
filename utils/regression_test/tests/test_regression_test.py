...
class TestRegressionTest(unittest.TestCase):
    ...
    def test_run_regression_test_fail(self):
        # Setup
        test = RegressionTest('fail_run', 'aperi-mech', 1, ['bad_input.yaml'])
    ...
    def test_run_exodiff_check_fail(self):
        # Setup
        test = RegressionTest('fail_exodiff', 'aperi-mech', 1, ['input.yaml'])
        exodiff = ExodiffCheck('fail_exodiff', 'exodiff', 'compare.exodiff', 'results.exo', 'bad_gold.exo', [])
